...
peacock-tee-deploy/
  ...
  js/core/yarns.js              ← per-project yarn colours for an imported chart's colour slots
...
**New project → Import pattern** takes a pattern CSV (`docs/pattern-csv-template.md`) or a chart export `.stitchchart.json` (`docs/stitchchart-import.md`); `handlePatternFileText` decides by content. Both become **custom patterns** in `pt3_custom_patterns` — never committed, so bought patterns are fine. A chart import shows a preview sheet first (name, thumbnail, stitch counts, flat/round, RS/WS row 1) and becomes one `hasChart` phase. Colours are **yarn slots**, not paint: `phase.chartColors` is a grid of slot indices parallel to the chart (kept separate so nothing that reads `CHART_B` changes), and `phase.chartYarns` is `[{color, name}]` — named in the preview ("Front" / "Back"), the export's colour only a default. The knitter picks the real yarn **per project** from the tappable chips under the section name (`openYarnSheet`); stored in `pt3_proj_<id>_yarns`, **local-only** like `midRowPos`, written directly rather than via `save()` so it never enqueues a progress push. Cells only reference `var(--yarn-N)`; `applyYarnVars()` sets those on `:root`, so a colour change repaints with no re-render. The export's `referenceImage` is never stored. An unknown stitch id blocks the import: **ask before adding** to `STITCHCHART_IDS`/`GLOSSARY`.
